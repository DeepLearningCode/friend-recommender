...
class FriendGraph(object):
    ...
    def make_network(self, df):
        ...
        G = nx.from_pandas_dataframe(df, df.columns[0], df.columns[1])
        return G
...
def run():
    """Load data, create networkx graph, and print friend recommendations"""
    
    if len(sys.argv) is 2: # user_id is input as command line argument
        user_id = int(sys.argv[1]) # e.g. python friend_rec.py 59003
